...
int canal = 0 , resolucaoo = 8 , pino = 26 , freqq = 10000; 

#define Num_Samples  112
#define MaxWaveTypes 4 

unsigned long tF, tI = 0;
...
void medicoes(){
    float dCycle = 0;
    double taxaAmostragem = 1/(111*freqq)*10^6;
    Serial.println("Entrei!");
    for(int i = 0; i < 112 ; i++){
        tF = micros();
        dCycle = 0.1 * WaveFormTable[0][i];
        Serial.println(dCycle);
        //Serial.println(millis());
        while(tF - tI < taxaAmostragem){
...
void pwm(){
    ...
    ledcSetup(canal, freqq, resolucaoo);
    ledcWrite(canal, 254);
    attachInterrupt(pino, medicoes, FALLING );
    Serial.println("Criei a interrupção!");
}